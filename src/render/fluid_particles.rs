...
impl ParticleFluid {
    pub fn new(gpu: &Gpu, scene_layout: &wgpu::BindGroupLayout) -> Self {
        ...
        let group=|a:&wgpu::Buffer,b:&wgpu::Buffer,c:&wgpu::Buffer,d:&wgpu::Buffer|gpu.device.create_bind_group(&wgpu::BindGroupDescriptor{label:Some("fluid-particle-bindings"),layout:&layout,entries:&[
            ...
            wgpu::BindGroupEntry{binding:4,resource:params.as_entire_binding()},wgpu::BindGroupEntry{binding:5,resource:colliders.as_entire_binding()},wgpu::BindGroupEntry{binding:6,resource:grid_counts.as_entire_binding()},wgpu::BindGroupEntry{binding:7,resource:grid_particles.as_entire_binding()},wgpu::BindGroupEntry{binding:8,resource:density.as_entire_binding()},wgpu::BindGroupEntry{binding:9,resource:emitter_state.as_entire_binding()}]});
        ...
        Self{pipeline,render_pipeline,groups,render_groups,current:0,params,colliders,grid_counts,grid_particles,clear_pipeline,insert_pipeline,density_pipeline,density,emit_accumulator:0.0,emitter_state}
    }
    ...
    pub fn update(&mut self,gpu:&Gpu,encoder:&mut wgpu::CommandEncoder,world:Option<&FluidWorld>){
        ...
        gpu.queue.write_buffer(&self.emitter_state,0,bytemuck::cast_slice(&[0u32,emit_count,0u32,0u32]));
        let p=Params{
